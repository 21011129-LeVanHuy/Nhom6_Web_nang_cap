  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Gửi yêu cầu đến API với user_id
        const response = await axios.get(
          `http://localhost:8000/api/add-to-cart-get-products?user_id=${userId}`
        );
    ...
    if (paymentMethod === "direct") {
      ...
      try {
        ...
        if (response.data.code === "00") {
          ...
          console.error("Error in payment response:", response.data.message);
        }
      } catch (error) {
                    ...
                    {cart.map((item) => (
                      <div className="row gy-3 mb-4" key={item.id}>
                        ...
                        <div className="col-lg-2 col-sm-6 col-6 d-flex flex-row flex-lg-column flex-xl-row text-nowrap">
                          ...
                          <div style={{ marginTop: 35 }} className="ms-5">
                            <text className="h6">
                              {" "}
