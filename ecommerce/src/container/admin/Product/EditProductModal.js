  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.categories_id == 1) {
        setCpu(product.details.CPU || "");
        setRam(product.details.RAM || "");
        setHardware(product.details.HARDWARE || "");
        setCard(product.details.CARD || "");
        setMonitor(product.details.MONITOR || "");
        setPin(product.details.PIN || "");
        setWeight(product.details.WEIGHT || "");
        setMaterial(product.details.MATERRIAL || "");
        setLength(product.details.LENGHT || "");
      }
    }
  }, [product]);
